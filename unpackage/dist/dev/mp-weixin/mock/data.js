...
const userList = Array.from({ length: 10 }, () => ({
  ...
  userName: common_vendor.Mock.Random.cname(),
  userAvatar: "/static/avatar/user" + common_vendor.Mock.Random.integer(1, 5) + ".png",
  phone: common_vendor.Mock.Random.string("number", 11),
  // 随机生成11位手机号
  email: common_vendor.Mock.Random.email(),
  // 随机生成邮箱
  password: common_vendor.Mock.Random.string(8)
  // 随机生成8位密码
}));
userList.push({
  // 添加一个测试用户
...
console.log(userList);
const commentsData = {};
Object.keys(goodsList).forEach((id) => {
  commentsData[id] = Array.from({ length: 5 }, () => {
    const user = userList[common_vendor.Mock.Random.integer(0, userList.length - 1)];
    return {
      id: common_vendor.Mock.Random.id(),
      goodsId: parseInt(id),
      userId: user.userId,
      userAvatar: user.userAvatar,
      userName: user.userName,
      rating: common_vendor.Mock.Random.integer(1, 5),
      content: common_vendor.Mock.Random.paragraph(1, 3),
